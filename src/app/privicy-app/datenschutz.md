# Datenschutzerklärung (Privacy Policy)

**Stand:** 28. April 2026

EgalDeutsch ("wir", "unser" oder "uns") betreibt die mobile Anwendung EgalDeutsch. Diese Seite informiert Sie über unsere Richtlinien zur Erhebung, Verwendung und Weitergabe personenbezogener Daten bei der Nutzung unseres Dienstes.

## 1. Datenerhebung & Verwendung

### a. Personenbezogene Daten
Während der Nutzung unseres Dienstes bitten wir Sie möglicherweise, uns bestimmte personenbezogene Daten bereitzustellen, darunter:
*   E-Mail-Adresse
*   Vorname und Nachname
*   Profilbild (via Google Login)
*   Sprachpräferenzen

### b. Log-Daten
Wir erfassen Informationen, die Ihr Gerät sendet, wie IP-Adresse, Gerätetyp, eindeutige Gerätekennungen und andere Diagnosedaten zur Gewährleistung der App-Sicherheit und Kernfunktionen.

## 2. Berechtigungen & Audiodaten
Für das Aussprache-Coaching benötigen wir Zugriff auf Ihr **Mikrofon (RECORD_AUDIO)**.
*   Die Verarbeitung erfolgt via **Google ML Kit** für direktes Feedback.
*   Wir speichern keine Audioaufnahmen dauerhaft auf unseren Servern, sofern dies nicht ausdrücklich für Support-Anfragen erforderlich ist.

## 3. Aufbewahrung von Daten
Wir bewahren Ihre personenbezogenen Daten nur so lange auf, wie dies für die in dieser Datenschutzerklärung dargelegten Zwecke erforderlich ist. Daten werden gelöscht, sobald Sie Ihr Konto löschen oder die gesetzlichen Aufbewahrungsfristen abgelaufen sind.

## 4. Drittanbieter
Wir beauftragen folgende Drittanbieter mit der Bereitstellung von Diensten:
*   **Google Cloud Platform:** Hosting und Infrastruktur (Google Cloud Run).
*   **Google ML Kit:** Lokale Spracherkennung auf dem Gerät.
*   **Google Play Services:** App-Distribution und Core-APIs.

## 5. Privatsphäre von Kindern
Unser Dienst richtet sich nicht an Personen unter 13 Jahren. Wir sammeln wissentlich keine personenbezogenen Daten von Kindern unter 13 Jahren. Sollten wir erfahren, dass ein Kind uns Daten übermittelt hat, werden wir diese umgehend löschen.

## 6. Ihre Rechte (DSGVO)
Sie haben im Rahmen der DSGVO jederzeit das Recht auf Auskunft, Berichtigung und Löschung Ihrer Daten (Art. 15-17 DSGVO).

## 7. Datenlöschung
Sie können die Löschung Ihres Kontos und aller damit verbundenen Daten jederzeit anfordern.
*   **E-Mail:** Senden Sie eine Anfrage an **egaldeutsch.com@gmail.com**. Wir bearbeiten diese Anfragen innerhalb von 30 Tagen.

## 8. Verantwortliche Stelle
**Steve Phan**  
Dorfstraße 35P  
13057 Berlin, Deutschland  
E-Mail: egaldeutsch.com@gmail.com

## 9. Änderungen dieser Richtlinie
Wir können unsere Datenschutzerklärung von Zeit zu Zeit aktualisieren. Wir werden Sie über Änderungen informieren, indem wir die neue Version auf dieser Seite veröffentlichen.

## 10. Kontakt
E-Mail: [egaldeutsch.com@gmail.com](mailto:egaldeutsch.com@gmail.com)  
Website: [https://egaldeutsch.com](https://egaldeutsch.com)
