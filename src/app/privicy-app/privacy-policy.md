# Privacy Policy for EgalDeutsch

**Effective Date:** April 28, 2026  
**Last Updated:** April 28, 2026

EgalDeutsch ("we," "our," or "us") operates the EgalDeutsch mobile application. This page informs you of our policies regarding the collection, use, and disclosure of personal data when you use our Service and the choices you have associated with that data.

## 1. Information Collection & Use

### a. Personal Data
While using our Service, we may ask you to provide us with certain personally identifiable information that can be used to contact or identify you ("Personal Data"). This includes:
*   Email address
*   First name and last name
*   Profile picture (via Google Login)
*   Language preferences

### b. Usage & Log Data
We may also collect information that your mobile device sends whenever you visit our Service. This may include your IP address, device type, unique device identifiers, and other diagnostic data to ensure app security and core functionality.

## 2. Permissions & Audio Data
To provide the core language coaching features, we request **Microphone (RECORD_AUDIO)** permission.
*   Audio is processed using **Google ML Kit** for real-time pronunciation feedback.
*   We do not store audio recordings permanently on our servers unless specifically required for a requested support case.

## 3. Retention of Data
We will retain your Personal Data only for as long as is necessary for the purposes set out in this Privacy Policy. We will retain and use your data to the extent necessary to comply with our legal obligations (for example, if we are required to retain your data to comply with applicable laws), resolve disputes, and enforce our legal agreements and policies.

## 4. Service Providers
We may employ third-party companies and individuals to facilitate our Service:
*   **Google Cloud Platform:** Hosting and backend infrastructure.
*   **Google ML Kit:** On-device machine learning for speech recognition.
*   **Google Play Services:** App distribution and core Android APIs.

## 5. Children's Privacy
Our Service does not address anyone under the age of 13. We do not knowingly collect personally identifiable information from anyone under the age of 13. If you are a parent or guardian and you are aware that your child has provided us with Personal Data, please contact us.

## 6. Your Rights (GDPR)
Under the **GDPR**, you have the right to access, update, or delete your personal data at any time. To exercise these rights, please contact us at the email below.

## 7. Data Deletion
Users can request the deletion of their account and all associated data at any time.
*   **Web Request:** Use our [Account Deletion Request Page](https://egaldeutsch.com/delete-account).
*   **Email:** Send a request to **egaldeutsch.com@gmail.com**. We will process these requests within 30 days.

## 8. Data Controller
**Steve Phan**  
Dorfstraße 35P  
13057 Berlin, Germany  
Email: egaldeutsch.com@gmail.com

## 9. Changes to This Privacy Policy
We may update our Privacy Policy from time to time. We will notify you of any changes by posting the new Privacy Policy on this page and updating the "Last Updated" date.

## 10. Contact Us
Email: [egaldeutsch.com@gmail.com](mailto:egaldeutsch.com@gmail.com)  
Website: [https://egaldeutsch.com](https://egaldeutsch.com)
