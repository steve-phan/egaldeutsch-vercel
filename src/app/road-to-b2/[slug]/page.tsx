...
import {
  ...
  getRoadToB2AdjacentChapters,
  getRoadToB2ChapterBySlug,
  getRoadToB2Html,
  getRoadToB2Slugs,
} from "@/lib/road-to-b2";
import { getRoadToB2Quiz } from "@/lib/road-to-b2-quizzes";
import { RoadToB2Quiz } from "@/components/road-to-b2/road-to-b2-quiz";

export async function generateStaticParams() {
  return getRoadToB2Slugs().map((slug) => ({
...
export async function generateMetadata({ params }: Props) {
  ...
  const chapter = getRoadToB2ChapterBySlug(resolvedParams.slug);

  if (!chapter) {
    return {
      title: "Road to B2 Chapter Not Found",
    };
  }

  return {
    title: `Kapitel ${chapter.chapterNumber}: ${chapter.title} | Road to B2 Deutsch`,
    description: chapter.description,
    alternates: {
      canonical: `/road-to-b2/${chapter.slug}`,
    },
  };
}

export default async function RoadToB2ChapterPage({ params }: Props) {
  const resolvedParams = await params;
  const chapter = getRoadToB2ChapterBySlug(resolvedParams.slug);
  ...
  const { previous, next } = getRoadToB2AdjacentChapters(chapter.slug);
  const contentHtml = await getRoadToB2Html(chapter.contentHtml);
  const quiz = getRoadToB2Quiz(chapter.slug);
